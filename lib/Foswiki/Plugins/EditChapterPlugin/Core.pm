...
sub new {
    ...
    Foswiki::Func::addToZone( 'head', 'EDITCHAPTERPLUGIN',
...
HERE
    Foswiki::Func::addToZone( 'script', 'EDITCHAPTERPLUGIN',
        <<'HERE', 'JQUERYPLUGIN::FOSWIKI, JQUERYPLUGIN::HOVERINTENT' );
...
sub jsonRpcUnlockTopic {
    ...
    if ( $wikiName ne $currentWikiName ) {
        throw Foswiki::Contrib::JsonRpcContrib::Error( 500,
            "Can't clear lease of user $wikiName" )
...
sub commonTagsHandler {
    ...
    my $insideInclude =
         $_[3]
      || Foswiki::Func::getContext()->{insideInclude}
      || 0;
    my $key = $web . '.' . $topic;
  ...
  /ge;
    ...
    unless ( defined $this->{enabled}{$key} ) {
        my $access =
          Foswiki::Func::checkAccessPermission( 'edit', $this->{wikiName},
    ...
    my $chapterNumber = 0;
    $text =~
s/(^)(---+[\+#]{$this->{minDepth},$this->{maxDepth}}[0-9]*(?:!!)?)(?!$this->{translationToken})([^\+#!].+?)($)/
    $1.
...
###############################################################################
sub handleSection {
    my ( $this, $web, $topic, $chapterNumber, $heading, $before, $after,
        $enabled )
      = @_;
    ...
    unless ($enabled) {
        ...
        $args{redirectto} =
          Foswiki::Func::getScriptUrl( $this->{baseWeb}, $this->{baseTopic},
            'view' )
          . $queryString . "#$id";

        my $url =
          Foswiki::Func::getScriptUrl( 'RenderPlugin', 'template', 'rest',
            %args );
...
sub handleEXTRACTCHAPTER {
    ...
    writeDebug(
        "thisWeb=$thisWeb, thisTopic=$thisTopic, theFrom=$theFrom, theTo=$theTo"
    );
    ...
    my $access =
      Foswiki::Func::checkAccessPermission( 'view', $this->{wikiName}, $text,
        $thisTopic, $thisWeb, $meta );
    ...
    while ( $text =~ /(^.*$)/gm ) {
        ...
        if ( $line =~ /^%STARTCHAPTER%$/m ) {
            if ($insideChapter) {
                ...
                $startChapterPos =
                  pos($text) + 1;    # remember previous STARTCHAPTER
                ...
                if ( $extractionMode == 2 && defined($toPos) ) {
                    writeDebug(
                        "stopping at STARTMARK as we are in before-mode");
                    last;
                }
            }
        }
        ...
        if ( $line =~ /^%(STOP|END)CHAPTER%$/m ) {
            if ($insideChapter) {
                ...
                $stopChapterPos =
                  pos($text) - length($line);    # remember previous STOPCHAPTER
        ...
        if ( $line =~
            /^---+[\+#]{$this->{minDepth},$this->{maxDepth}}(?:!!)?\s*(.+?)$/m )
        {
            $chapterNumber++;
    ...
    # continue in before-mode; instead scan til end of text or STARTCHAPTER mark
                if ( $extractionMode == 2 ) {
                    writeDebug(
                        "... continuing searching for the next STARTCHAPTER");
                }
                else {
